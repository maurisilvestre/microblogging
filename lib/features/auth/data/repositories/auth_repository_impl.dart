...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<Either<Failure, User>> _auth(
    _EmailOrGoogle emailOrGoogle,
  ) async {
    if (await networkInfo.isConnected) {
      try {
        final remoteTrivia = await emailOrGoogle();
  ...
  Future<Either<Failure, User>> authEmailPassword(
    ...
    return await _auth(() =>
        authDataSource.authEmailPassword(email: email, password: password));
  }

  @override
  Future<Either<Failure, User>> authGoogle() async {
